...
datasetLoc = "./asset/dataset.csv"
# dataset = pd.read_csv(datasetLoc)
dataset = pd.read_csv(datasetLoc, encoding="latin1")
lb = 0 # lower bound
ub = 1000 # upper bound
...
if len(a) > 0:
    ...
    headers = ['0', '1', 'mutant', 'template', 'occupation']
    preparedOccDataTemp = [outputDataOcc['sentiment'], outputDataOcc['mutant'], outputDataOcc['mutant'], outputDataOcc['template'], outputDataOcc['placeholder']]
...
if len(b) > 0:
    ...
    headers = ['0', '1', 'mutant', 'template', 'age']
    preparedAgeDataTemp = [outputDataAge['sentiment'], outputDataAge['mutant'], outputDataAge['mutant'], outputDataAge['template'], outputDataAge['age']]
    preparedOccData = pd.concat(preparedAgeDataTemp, axis = 1, keys = headers) 
    preparedOccData.to_csv('./prepared/age.csv', index=False)
